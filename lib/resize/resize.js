...
export class Resize {
  constructor(
    ...
    this.resizeCallback = (typeof resizeCallback == "function")
      ? resizeCallback
      : function () {};
    this.targetWidthMultipliedByChannels = this.targetWidth *
      this.colorChannels;
    this.originalWidthMultipliedByChannels = this.widthOriginal *
      this.colorChannels;
    this.originalHeightMultipliedByChannels = this.heightOriginal *
      this.colorChannels;
    this.widthPassResultSize = this.targetWidthMultipliedByChannels *
      this.heightOriginal;
    this.finalResultSize = this.targetWidthMultipliedByChannels *
      this.targetHeight;
    this.initialize();
  }
  initialize() {
    //Perform some checks:
    if (
      this.widthOriginal > 0 && this.heightOriginal > 0 &&
      this.targetWidth > 0 && this.targetHeight > 0
    ) {
      if (this.useWebWorker) {
        this.useWebWorker = this.widthOriginal != this.targetWidth ||
  ...
  configurePasses() {
    if (this.widthOriginal == this.targetWidth) {
      ...
      if (this.ratioWeightWidthPass < 1 && this.interpolationPass) {
        ...
        this.resizeWidth = (this.colorChannels == 4)
    ...
    if (this.heightOriginal == this.targetHeight) {
      ...
      if (this.ratioWeightHeightPass < 1 && this.interpolationPass) {
        ...
        this.resizeHeight = (this.colorChannels == 4)
  ...
  resizeWidthRGB(buffer) {
    ...
    let pixelOffset = 0;
    let outputOffset = 0;
    const nextLineOffsetOriginalWidth = this.originalWidthMultipliedByChannels -
      2;
    const nextLineOffsetTargetWidth = this.targetWidthMultipliedByChannels - 2;
    ...
    do {
      ...
      do {
        ...
        weight > 0 && actualPosition < this.originalWidthMultipliedByChannels
      );
      for (
  ...
  resizeWidthInterpolatedRGB(buffer) {
    ...
    weight -= 0.25;
    for (
      const interpolationWidthSourceReadStop = this.widthOriginal - 1;
      ...
      for (
        ...
        outputBuffer[finalOffset] = (buffer[pixelOffset] * firstWeight) +
  ...
  resizeWidthRGBA(buffer) {
    ...
    const nextLineOffsetOriginalWidth = this.originalWidthMultipliedByChannels -
      3;
    const nextLineOffsetTargetWidth = this.targetWidthMultipliedByChannels - 3;
    const output = this.outputWidthWorkBench;
    const outputBuffer = this.widthBuffer;
    do {
      for (line = 0; line < this.originalHeightMultipliedByChannels;) {
  ...
  resizeHeightInterpolated(buffer) {
    ...
    weight -= 0.25;
    for (
      const interpolationHeightSourceReadStop = this.heightOriginal - 1;
      ...
      firstWeight = 1 - secondWeight;
      //Interpolate:
      pixelOffsetAccumulated = Math.floor(weight) *
        this.targetWidthMultipliedByChannels;
      pixelOffsetAccumulated2 = pixelOffsetAccumulated +
        this.targetWidthMultipliedByChannels;
      for (
        pixelOffset = 0;
        pixelOffset < this.targetWidthMultipliedByChannels;
        ...
        outputBuffer[finalOffset++] =
          (buffer[pixelOffsetAccumulated + pixelOffset] * firstWeight) +
          (buffer[pixelOffsetAccumulated2 + pixelOffset] * secondWeight);
      }
    }
    //Handle for only one interpolation input being valid for end calculation:
    while (finalOffset < this.finalResultSize) {
      for (
        pixelOffset = 0,
          pixelOffsetAccumulated = interpolationHeightSourceReadStop *
            this.targetWidthMultipliedByChannels;
        pixelOffset < this.targetWidthMultipliedByChannels;
        ++pixelOffset
      ) {
